...
/obj/structure/toilet/attack_hand(mob/living/user as mob)
	if(swirlie)
		...
		usr.visible_message(span("danger", "[user] slams the toilet seat onto [swirlie.name]'s head!"),
							span("notice", "You slam the toilet seat onto [swirlie.name]'s head!"),
...
/obj/structure/toilet/attackby(obj/item/I as obj, mob/living/user as mob)
	if(I.iscrowbar())
		...
		if(do_after(user, 30/I.toolspeed))
			user.visible_message(span("notice", "[user] [cistern ? "replaces the lid on the cistern" : "lifts the lid off the cistern"]!"),
								 span("notice", "You [cistern ? "replace the lid on the cistern" : "lift the lid off the cistern"]!"),
		...
		if(isliving(G.affecting))
			...
			if(G.state>1)
				if(!GM.loc == get_turf(src))
					...
					return
				if(open && !swirlie)
					user.visible_message(span("danger", "[user] starts to give [GM.name] a swirlie!"),
										 span("notice", "You start to give [GM.name] a swirlie!"))
					swirlie = GM
					if(do_after(user, 30, 5, 0))
						user.visible_message(span("danger", "[user] gives [GM.name] a swirlie!"),
											 span("notice", "You give [GM.name] a swirlie!"),
					...
					user.visible_message(span("danger", "[user] slams [GM.name] into the [src]!"),
										 span("notice", "You slam [GM.name] into the [src]!"))
...
/obj/machinery/shower/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(I.iswrench())
		...
		if(do_after(user, 50/I.toolspeed))
			watertemp = newtemp
			user.visible_message(span("notice", "[user] adjusts the shower with \the [I]."),
...
/obj/structure/sink/attack_hand(mob/user as mob)
	...
	user.clean_blood()
	user.visible_message(span("notice", "[user] washes their hands using \the [src]."), \
...
/obj/structure/sink/attackby(obj/item/O as obj, mob/user as mob)
	...
	if (istype(RG) && RG.is_open_container())
		...
		switch(atype)
			...
			if ("Empty")
				...
				user.visible_message(span("notice", "[usr] empties [empty_amount]u of \the [RG] into \the [src]."),
	...
	O.clean_blood()
	user.visible_message(span("notice", "[user] washes \a [I] using \the [src]."), \
